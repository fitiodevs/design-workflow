...
| `theme-create` | Composer | Pigma | Builds palette from scratch (OKLCH-uniform, anti-AI-slop) | `/Composer`, `/Pigma`, `/pigma` |
| `theme-extend` | Surgeon | Mira | Adds or fixes a single token with WCAG-validated light/dark pair | `/Surgeon`, `/Mira`, `/mira` |
| `theme-port` | Architect | Elo | Ports a Figma frame OR an HTML mockup (any source) into widgets using your tokens | `/Architect`, `/Elo`, `/elo` |
| `theme-critique` | Critic | Olavo | Nielsen 10 + AI-slop verdict + persona walkthrough | `/Critic`, `/Olavo`, `/olavo` |
| `theme-bolder` | Amplifier | Saga | Amplifies a timid screen | `/Amplifier`, `/Saga`, `/saga` |
| `theme-quieter` | Refiner | Calma | Calms a shouty screen | `/Refiner`, `/Calma`, `/calma` |
| `theme-distill` | Distiller | Cerne | Cuts decisions to ≤ 4 | `/Distiller`, `/Cerne`, `/cerne` |
...
| `flow` | Flow auditor | Flavio | Cross-screen UX flow audit against `docs/flow-heuristics.md` | `/Flow`, `/Flavio`, `/flavio`, `/flow` |
...
| `tweaks` | Tweaker | Twik | Wraps any tweaks-ready HTML mockup with a side panel of live CSS-custom-property knobs (accent hue · type scale · density · theme mode · motion) persisting to localStorage; refuses non-tweaks-ready inputs and recommends `/frontend-design` refit | `/tweaks`, `/Twik`, `/twik`, `/Tweaker` |
