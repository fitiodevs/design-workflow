...
### T-06 🅿️ Update `frontend-design/SKILL.md` body emission rules
...
### T-07 🅿️ Update Clara's checklist
...
### T-08 ⬜ Update Clara's eval (if exists)
...
### T-14 ⬜ Update README §What changed in v1.3.0
...
- **Verify:** `grep -c "What changed in v1.3.0" README.md` = 1; pipeline diagram present (look for "/tweaks" between "/frontend-design" and "/theme-port").
- **Refs:** REQ-04.1
...
### T-17 ⬜ Append D-15, D-16, D-17 to STATE.md
- **Action:** add 3 decisions per spec REQ-05.
- **Verify:** `grep -c "^- \*\*D-1[567]" .specs/project/STATE.md` = 3.
- **Refs:** REQ-05

### T-18 ⬜ Update `marketplace.json`
- **Action:** add `./skills/tweaks` to skills array; bump version `1.2.0` → `1.3.0`; update top-level description to mention "20 atomic operators + interactive mockup stage" (or current count).
- **Verify:** `jq -r '.metadata.version' .claude-plugin/marketplace.json` = `1.3.0`; `jq '.plugins[0].skills | length' .claude-plugin/marketplace.json` = 20.
- **Refs:** REQ-06.3
...
- **Verify:** all 7 steps documented in a manual log file `.design-spec/smoke/v1.3.0-2026-XX-XX.md`. Any browser-specific issues caught surface as new tasks (or get fixed inline if simple).
...
- **Action:**
  ...
  git commit -m "feat(release): v1.3.0 — interactive mockup stage (tweaks + 5dim critique)"
...
2. `git tag | grep pre-v1.3.0` — se ausente, T-00
