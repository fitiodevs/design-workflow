...
**Tag pre-rollback:** `pre-v1.3.0` (criar em T-00)
**Target:** v1.3.0
...
- **Action:**
  ...
  echo "Upstream SHA: $UPSTREAM_SHA" > /tmp/v1.3.0-source.txt
  ```
- **Verify:** `git tag | grep pre-v1.3.0` returns match; SHA captured.
- **Refs:** spec REQ-01
...
#### Confirmed slugs (T-01 verified 2026-05-07 vs upstream SHA `d4b547c`)

| Category | Slugs |
|---|---|
| AI & LLM (3) | `claude`, `cohere`, `mistral-ai` |
| Dev Tools (3) | `linear-app`, `vercel`, `raycast` |
| Productivity (2) | `notion`, `cal` |
| Backend / Data (2) | `supabase`, `sentry` |
| Design / Creative (2) | `figma`, `framer` |
| Fintech (2) | `stripe`, `revolut` |
| E-Commerce (2) | `airbnb`, `nike` |
| Media (2) | `apple`, `spotify` |
| Automotive (1) | `tesla` |
| Editorial (1) | `atelier-zero` |

20/20 returned `name` from `gh api`. No swaps needed.
...
### T-02 ✅ Fork AI&LLM trio (claude, cohere, mistral-ai)
...
### T-03 ✅ Fork Dev Tools trio (linear-app, vercel, raycast)
- Same pattern.

### T-04 ✅ Fork Productivity pair (notion, cal)
- Same pattern.

### T-05 ✅ Fork Backend/Data pair (supabase, sentry)
- Same pattern.

### T-06 ✅ Fork Design/Creative pair (figma, framer)
- Same pattern.

### T-07 ✅ Fork Fintech pair (stripe, revolut)
- Same pattern.

### T-08 ✅ Fork E-Commerce pair (airbnb, nike)
- Same pattern.

### T-09 ✅ Fork Media/Consumer pair (apple, spotify)
- Same pattern.

### T-10 ✅ Fork Automotive solo (tesla)
- Same pattern.

### T-11 ✅ Fork Editorial solo (atelier-zero)
...
### T-24 ✅ Append D-17 + D-18 to `.specs/project/STATE.md`
- **Note:** spec called D-13 + D-14, but those slots are taken (D-13 = v1.1.2 install bug, D-14 = v1.2.0 adapter contract). Renumbered to next-available D-17 + D-18.
- **Action:**
  - **D-17 — DESIGN.md library**: 20 entries curated from upstream open-design's 71, attribution chain per file, sync manual at SHA `d4b547c`.
  - **D-18 — Translator semantics**: inference chain priority (per-entry name override > section-level > description hint > WCAG-corrected fallback); failed mappings always flagged, never silent; 7 game/badge tokens flagged for user input.
- **Verify:** `grep -c "^- \*\*D-1[78]" .specs/project/STATE.md` = 2.
- **Refs:** REQ-05

### T-25 ✅ Add §What changed in v1.3.0 + Inspiration library section to README
- **Action:** new release section above v1.2.1; new top-level "## Inspiration library" with 20-entry category table.
- **Verify:** `grep -c "What changed in v1.3.0" README.md` = 1; `grep -c "Inspiration library" README.md` ≥ 1; table has 9 categories.
- **Refs:** REQ-06.1, REQ-06.2

### T-26 ✅ Bump `marketplace.json`
- **Action:** `1.2.1` → `1.3.0` via Edit on `version`.
- **Verify:** `jq -r '.metadata.version' .claude-plugin/marketplace.json` = `1.3.0`.
- **Refs:** REQ-07.3
...
- **Action:**
  ...
  git commit -m "feat(release): v1.3.0 — DESIGN.md inspiration library + translator"
...
2. Confirma `pre-v1.3.0` tag exists; senão volta pro T-00
