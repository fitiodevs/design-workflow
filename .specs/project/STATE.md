...
- **D-23 — Schools library separate from brands library — different abstraction, different folder, different translator.** `design-systems-schools/` (12 entries, `SCHOOL.md`) is a sibling to `design-systems/` (20 entries, `DESIGN.md`), not a sub-folder. **Why:** brands encode *identities* ("look like Apple"); schools encode *philosophies* ("follow Müller-Brockmann's discipline"). The same school applied to two projects produces two different palettes; the same brand fork produces the same hex codes. Mixing them in one folder confuses both the picker UX and the translator logic. File naming reflects the split: `DESIGN.md` (brands) vs `SCHOOL.md` (schools); at a glance the user knows which abstraction they're touching. The school library covers 12 schools across 4 scenario-strength categories (Flutter-strong: Pentagram / IA / Brutalism / Locomotive; Print-strong: Müller-Brockmann / Kenya Hara / Editorial / Atelier-Zero; AI-gen-strong: Memphis / Active Theory; Versatile: Sagmeister-Walsh / Takram). Date: 2026-05-07.
- **D-24 — School entries authored from scratch under Apache-2.0 — huashu-design license forbids fork.** The structural idea (a 7-column scenario matrix per school with star ratings + recommended approaches) is inspired by `alchaincyf/huashu-design`'s `references/design-styles.md` (Personal Use Only). All 12 SCHOOL.md files are written from scratch in our own voice, citing huashu as idea source in the attribution header. Atelier Zero is the exception — hand-curated by `nexu-io/open-design` (Apache-2.0), included as the 12th entry to round out the studio-collage register. **Why this matters:** licensing fragility is real; if huashu ever changes terms, our work is unaffected because we don't fork. Trade-off: more authoring work upfront; benefit: durable Apache-2.0 lineage. Date: 2026-05-07.
- **D-25 — Multi-target matrix is reference-only for non-Flutter rows.** The 7-column matrix (Flutter UI · HTML · PPT · PDF · Infographic · Cover · AI-image-gen) tells users where each school fits, but design-workflow only ships executable paths for Flutter UI and HTML mockup. The other 5 columns recommend external tools (`docs/design-schools-execution-paths.md`): PPT via Marp/Slidev, PDF via Pandoc/Typst/Prince, Infographic via Puppeteer+D3, Cover via HTML→Figma, AI-image-gen via Midjourney/SDXL/Runway/Sora. **Why this design:** we don't pretend to be a multimedia generator; the matrix is a routing recommendation, not a commitment. External-tool recommendations are time-stamped and re-validated quarterly because tools rot. Pattern is **HTML-as-canonical-source** — invest in `frontend-design --school` because HTML is the upstream of almost every downstream scenario. Date: 2026-05-07.
